...
import 'package:musee/Bloc/moment_bloc.dart';
import 'package:musee/Bloc/visiter_bloc.dart';
import 'package:musee/Model/Moment.dart';
import 'package:musee/Screens/Visite/ShowVisiter.dart';
...
class _ListeVisitesState extends State<ListeVisites> implements AlertDialogCallback{
  ...
  void initState() {
    ...
    var listMo = momentBloc.getMoments().then((value){
      listMoment = value;
      jour = listMoment[0].jour.toString();
                            ...
                            child: DropdownButton<String>(
                              ...
                              items: listMoment.map((moment) {
                                return DropdownMenuItem<String>(
                                  value: moment.jour.toString(),
                                  child: SizedBox(
                                    width: 150, //expand here
                                    child: Text(
                                      moment.jour.toString(),
                                      style: const TextStyle(fontSize: 15),
                                      textAlign: TextAlign.end,
                                    ),
                                  ),
            ...
            actions: <Widget>[
              ...
              TextButton(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    setState(() {
                        delete(DateTime.parse(jour), museeNum);
                        Navigator.pop(context);
                    });
                  },
                  child: const Text(
  ...
  void save() {
      Visiter visiter= Visiter(
        jour: DateTime.parse(jour),
        numMus: museeNum, 
        nbvisiteurs: int.parse(txtNbVisiteurs.text.trim()),
      );
      if (saveOrUpdateText == 'Enregistrer'){
          visiterBloc.addVisiter(visiter);
      }else{
        print(visiter.jour);
        visiterBloc.updateVisiter(visiter);
      }
     
  }

  @override
  void delete(DateTime jour, int numMus) {
    visiterBloc.deleteVisiterById(jour: jour, numMus: numMus);
  }
}
